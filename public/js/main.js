...
(function () {
  ...
  if (lightbox) {
    ...
    // Auto-open a specific photo when arriving from a homepage preview link,
    // e.g. /portfolio.html?photo=portfolio-03
    var urlParams = new URLSearchParams(window.location.search);
    var photoParam = urlParams.get('photo');
    if (photoParam) {
      var matchIndex = galleryItems.findIndex(function (el) {
        return el.dataset.lightboxSrc && el.dataset.lightboxSrc.indexOf(photoParam) !== -1;
      });
      if (matchIndex > -1) {
        setTimeout(function () { openLightbox(matchIndex); }, 350);
      }
    }
  }

  /* ---------------- Video lightbox (click a video tile to play it large) ---------------- */
  var videoLightbox = document.querySelector('.video-lightbox');
  if (videoLightbox) {
    var vlbVideo = videoLightbox.querySelector('video');
    var vlbCaption = videoLightbox.querySelector('.video-lightbox-caption');
    var vlbCloseBtn = videoLightbox.querySelector('.video-lightbox-close');

    function openVideoLightbox(mp4Src, webmSrc, caption) {
      vlbVideo.innerHTML = '';
      if (webmSrc) {
        var s1 = document.createElement('source'); s1.src = webmSrc; s1.type = 'video/webm';
        vlbVideo.appendChild(s1);
      }
      if (mp4Src) {
        var s2 = document.createElement('source'); s2.src = mp4Src; s2.type = 'video/mp4';
        vlbVideo.appendChild(s2);
      }
      vlbVideo.load();
    ...
    document.querySelectorAll('[data-video-src]').forEach(function (el) {
      el.addEventListener('click', function (e) {
        e.preventDefault();
        openVideoLightbox(el.dataset.videoSrc, el.dataset.videoWebm, el.dataset.videoCaption);
      });
    });
    vlbCloseBtn && vlbCloseBtn.addEventListener('click', closeVideoLightbox);
    videoLightbox.addEventListener('click', function (e) { if (e.target === videoLightbox) closeVideoLightbox(); });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && videoLightbox.classList.contains('is-open')) closeVideoLightbox();
    });
  }

  /* ---------------- Wedding albums (portfolio page, filtered by ?album=) ---------------- */
  var ALBUM_NAMES = {
    'riya-aniket': 'Riya & Aniket',
    'debolina-sourav': 'Debolina & Sourav',
    'momita-arjun': 'Momita & Arjun',
    'priyanka-rohit': 'Priyanka & Rohit'
  };
  var albumBanner = document.querySelector('#album-banner');
  if (albumBanner) {
    var albumParams = new URLSearchParams(window.location.search);
    var albumSlug = albumParams.get('album');
    if (albumSlug && ALBUM_NAMES[albumSlug]) {
      var albumItems = document.querySelectorAll('.masonry-item');
      albumItems.forEach(function (item) {
        item.hidden = item.dataset.album !== albumSlug;
      });
      var filterBarEl = document.querySelector('.filter-bar');
      if (filterBarEl) filterBarEl.style.display = 'none';
      albumBanner.hidden = false;
      var nameEl = albumBanner.querySelector('#album-banner-name');
      if (nameEl) nameEl.textContent = ALBUM_NAMES[albumSlug];
      var countEl = albumBanner.querySelector('#album-banner-count');
      if (countEl) countEl.textContent = document.querySelectorAll('.masonry-item[data-album="' + albumSlug + '"]').length + ' photos';
    }
  }
